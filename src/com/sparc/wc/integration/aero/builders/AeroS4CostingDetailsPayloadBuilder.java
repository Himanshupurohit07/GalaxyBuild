...
/**
 * Builds an Costing Details payload for Aero PLM-to-S4 integration.<br>
 * 
 * FIXES/AMENDMENTS:<br>
 ...
 */
public class AeroS4CostingDetailsPayloadBuilder implements AeroCostingDetailsPayloadBuilder {
	...
	public AeroS4CostingDetailsPayloadBuilder(AeroCostingDetailsPayload costingPayload) {
	...
	private void buildCostSheetAttributes(Map<String, Object> costsheetPayload, LCSProductCostSheet flexCostSheet) throws WTException {
		...
		for (AeroPayloadAttribute attrDef : AeroPayloadAttributeDefinitions.
				getS4CostingAttributesDefinitionsForFlexType(AERO_COST_SHEET_ATTR_GROUP_KEY)) {
	...
	private void buildSourcingSeasonAttributes(Map<String, Object> costsheetPayload, LCSSourceToSeasonLink flexSourcingSeasonLink) throws WTException {
		...
		for (AeroPayloadAttribute attrDef : AeroPayloadAttributeDefinitions.
				getS4CostingAttributesDefinitionsForFlexType(AERO_SOURCING_SEASON_ATTR_GROUP_KEY)) {
	...
	private void buildSourcingConfigAttributes(Map<String, Object> costsheetPayload, LCSSourcingConfig flexSourcingConfig) throws WTException {
		...
		for (AeroPayloadAttribute attrDef : AeroPayloadAttributeDefinitions.
				getS4CostingAttributesDefinitionsForFlexType(AERO_SOURCING_CONFIG_ATTR_GROUP_KEY)) {
	...
	private void buildColorwayAttributes(Map<String, Object> costsheetPayload, LCSSKU flexColorway) throws WTException {
		...
		for (AeroPayloadAttribute attrDef : AeroPayloadAttributeDefinitions.getS4CostingAttributesDefinitionsForFlexType(AERO_COLORWAY_ATTR_GROUP_KEY)) {
	...
	private void buildSeasonAttributes(Map<String, Object> costsheetPayload, LCSSeason flexSeason) throws WTException {
		...
		for (AeroPayloadAttribute attrDef : AeroPayloadAttributeDefinitions.getS4CostingAttributesDefinitionsForFlexType(AERO_SEASON_ATTR_GROUP_KEY)) {
	...
	/**
	 * Builds the Costing Details payload for S4.
	...
	public AeroCostingDetailsPayload build() throws WTException, WTPropertyVetoException, Exception {
		...
		List<String> updatedSourceToSeasonLinks = new ArrayList<String>();
		...
		for (AeroCostingFlexTypeMaster costingMaster : flexCostingMasterList) {
			...
			LOGGER.debug("About to build Aero S4 Costing payload for cost sheet #" + costSheetNumber +
			...
			if (costingMaster.getProductCostSheet() != null && !updatedCostSheets.contains(costSheetNumber)) {
				...
				LOGGER.debug("About to update Aero S4 Costing integration flags for cost sheet #" + costSheetNumber + "(colorway #" + colorwayNumber + ").");
				
				try {
					
					if (AeroCostingRepository.updateIntegrationControlFlags(AeroProcessesParam.S4, costingMaster.getProductCostSheet())) {
						updatedCostSheets.add(costSheetNumber);
						LOGGER.debug("Aero S4 Costing integration flags for for cost sheet #" + costSheetNumber + "(colorway #" + colorwayNumber + ") have been updated.");
					}
					
				} catch(Exception csEx) {
					throw new Exception("Failed to update cost sheet S4 integration control flags for cost sheet #" + costSheetNumber + " under sourcing config #" + sourcingConfigNumber + " and product #" + productNumber + ".", csEx);
				}
				
			} else {
				LOGGER.debug("Aero S4 Costing integration flags update for for cost sheet #" + costSheetNumber + "(colorway #" + colorwayNumber + "). has been skipped (flags already updated).");
			}
			
			if (costingMaster.getSourcingSeasonLink() != null && !updatedSourceToSeasonLinks.contains(costingMaster.getSourcingSeasonLink().toString())) {
				
				LOGGER.debug("About to update Aero S4 Source-To-Season Link integration flag for cost sheet #" + costSheetNumber + "(colorway #" + colorwayNumber + ").");
				
				try {
					if (AeroCostingRepository.updateIntegrationControlFlags(costingMaster.getSourcingSeasonLink())) {
						updatedSourceToSeasonLinks.add(costingMaster.getSourcingSeasonLink().toString());
						LOGGER.debug("Aero S4 Source-To-Season Link integration flag for for cost sheet #" + costSheetNumber + "(colorway #" + colorwayNumber + ") has been updated.");
					}
				} catch(Exception srcEx) {
					throw new Exception("Failed to update sourcing-season S4 integration control flag for cost sheet #" + costSheetNumber + " of product #" + productNumber + " and season " + seasonName + ".", srcEx);
				}
				
			} else {
				LOGGER.debug("Aero S4 Source-To-Season Link flag update for for cost sheet #" + costSheetNumber + "(colorway #" + colorwayNumber + ") has been skipped (flag already updated).");
			}
