...
/**
 * Builds an Artile Details payload for Aero PLM-to-S4 integration.<br>
 * 
 * FIXES/AMENDMENTS:<br>
 ...
 */
public class AeroS4ArticleDetailsPayloadBuilder implements AeroArticleDetailsPayloadBuilder {
	...
	public AeroS4ArticleDetailsPayloadBuilder(AeroArticleDetailsPayload articlePayload) {
		this.articlePayload = articlePayload;
	}
	
	public AeroS4ArticleDetailsPayloadBuilder setColorwayNumber(Long colorwayNumber) {
	...
	private void buildProductSeasonAttributes(Map<String, Object> articleDetailsMap, LCSProductSeasonLink flexProductSeasonLink) throws WTException {
		...
		for (AeroPayloadAttribute attrDef : AeroPayloadAttributeDefinitions.
				getS4ArticleAttributesDefinitionsForFlexType(AERO_PRODUCT_SEASON_ATTR_GROUP_KEY)) {
	...
	private void buildColorwaySeasonAttributes(Map<String, Object> articleDetailsMap, LCSSKUSeasonLink flexColorwaySeasonLink) throws WTException {
		...
		for (AeroPayloadAttribute attrDef : AeroPayloadAttributeDefinitions.
				getS4ArticleAttributesDefinitionsForFlexType(AERO_COLORWAY_SEASON_ATTR_GROUP_KEY)) {
	...
	private void buildSourcingSeasonAttributes(Map<String, Object> articleDetailsMap, LCSSourceToSeasonLink flexSourcingSeasonLink) throws WTException {
		...
		for (AeroPayloadAttribute attrDef : AeroPayloadAttributeDefinitions.
				getS4ArticleAttributesDefinitionsForFlexType(AERO_SOURCING_SEASON_ATTR_GROUP_KEY)) {
	...
	private void buildProductAttributes(Map<String, Object> articleDetailsMap, LCSProduct flexProduct) throws WTException {
		...
		for (AeroPayloadAttribute attrDef : AeroPayloadAttributeDefinitions.getS4ArticleAttributesDefinitionsForFlexType(AERO_PRODUCT_ATTR_GROUP_KEY)) {
	...
	private void buildColorwayAttributes(Map<String, Object> articleDetailsMap, LCSSKU flexColorway) throws WTException {
		...
		for (AeroPayloadAttribute attrDef : AeroPayloadAttributeDefinitions.getS4ArticleAttributesDefinitionsForFlexType(AERO_COLORWAY_ATTR_GROUP_KEY)) {
			LOGGER.debug("[buildColorwayAttributes] Colorway attrDef to load: " + attrDef);
	...
	private void buildSeasonAttributes(Map<String, Object> articleDetailsMap, LCSSeason flexSeason) throws WTException {
		...
		for (AeroPayloadAttribute attrDef : AeroPayloadAttributeDefinitions.getS4ArticleAttributesDefinitionsForFlexType(AERO_SEASON_ATTR_GROUP_KEY)) {
	...
	/**
	 * Builds the Article Details payload for S4.
	 * @return The Article Details payload built.
	 * @throws WTException When an error occurs while accessing or retrieving the various data elements from PLM/Flex Objects.
	...
	public AeroArticleDetailsPayload build() throws WTException, WTPropertyVetoException, Exception {
		...
		for (AeroArticleFlexTypeMaster articleMaster : flexArticleMasterList) {
			...
			LOGGER.debug("About to build Aero S4 Article payload for colorway " + colorwayNumber + " at season " + seasonName + ".");
			...
			LOGGER.debug("About to update Aero S4 Article integration flags for colorway " + colorwayNumber + " at season " + seasonName + ".");
			
			try {
				AeroArticleRepository.updateIntegrationControlFlags(AeroProcessesParam.S4, articleMaster.getColorwaySeasonLink());
			} catch(Exception cslinkEx) {
				throw new Exception("Failed to update colorway-season S4 integration control flag for colorway #" + colorwayNumber + " and season " + seasonName + ".", cslinkEx);	
			}
			
			LOGGER.debug("Aero S4 Article integration flags for colorway " + colorwayNumber + " at season " + seasonName + " have been updated.");
