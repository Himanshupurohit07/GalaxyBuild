...
public class SPARCLoadMeasurementProduct {
	private static final Logger LOGGER = LogManager.getLogger(SPARCLoadMeasurementProduct.class.getName());
	...
	protected static LCSPointsOfMeasure getPOMTemplate(Hashtable<String, Object> dataValues, String fileName) {
		try {
			...
			if (searchTable.get("scnumber") != null) {
				scnumber = (String) searchTable.get("scnumber");
			}
			
			if (!scnumber.equals("") && !measurementName.equals("")
...
//			} 
	...
	protected static boolean createMeasurements(Hashtable<String, Object> dataValues, String fileName) {
		try {
			...
			System.out.println("data values inside the createMeasurements method  :: "+dataValues);
			...
			String productSizeName = LoadCommon.getValue(dataValues, "ProductSizeCategory", false);

			System.out.println("productSizeName :: "+productSizeName);
//			if ((value = LoadCommon.getValue(dataValues, "ProductSizeCategory", false)) != null) {
//				measurements.setProductSizeCategory(value);
//			}
			...
			System.out.println("Fetched  LCSMeasurements: " + measurements);

			// Fetch Product
			LCSProduct lp = (LCSProduct) VersionHelper.latestIterationOf(measurements.getProductMaster());
			ProductSizeCategory sizeCategory = getProductSizeCategory(lp,productSizeName);
			if (sizeCategory != null) {
				measurements.setProductSizeCategory(sizeCategory);
				measurements.setSampleSize(sizeCategory.getBaseSize());
				measurements.setSizeRun(sizeCategory.getSizeValues());
				System.out.println("Set ProductSizeCategory: " + sizeCategory.getName());
			} else {
				System.out.println("ProductSizeCategory is null, not set.");
			}
...
//				System.out.println("start of setting ProductSizeCategory");
//				// Fetch latest Measurements object from POM
//				LCSMeasurements lcm = (LCSMeasurements) VersionHelper.latestIterationOf(pom.getMeasurementsMaster());
//				System.out.println("Fetched LCSMeasurements: " + lcm);
//				// Fetch Product
//				LCSProduct lp = (LCSProduct) VersionHelper.latestIterationOf(lcm.getProductMaster());
//				// Get and set size category
//				ProductSizeCategory sizeCategory = getProductSizeCategory(lp);
//				if (sizeCategory != null) {
//					lcm.setProductSizeCategory(sizeCategory);
//					lcm.setSampleSize(sizeCategory.getBaseSize());
//					lcm.setSizeRun(sizeCategory.getSizeValues());
//				
//					System.out.println("Set ProductSizeCategory: " + sizeCategory.getName());
//				} else {
//					System.out.println("ProductSizeCategory is null, not set.");
//				}
	...
	public static ProductSizeCategory getProductSizeCategory(LCSProduct product,String name) throws WTException {
		System.out.println("-- getProductSizeCategory --");
		System.out.println("name :: "+name);
		ProductSizeCategory matchedSizeCat = null;
		SearchResults results = SizingQuery.findProductSizeCategoriesForProduct(product);
		if (results != null && results.getResultsFound() > 0) {
			Iterator prdSizeItr = results.getResults().iterator();
			while (prdSizeItr.hasNext()) {
				FlexObject fobj = (FlexObject) prdSizeItr.next();
				String prodSizeId = fobj.getString("PRODUCTSIZECATEGORY.IDA2A2");

				if (FormatHelper.hasContent(prodSizeId)) {
					ProductSizeCategory sizeCat = (ProductSizeCategory) LCSQuery.findObjectById(
						"com.lcs.wc.sizing.ProductSizeCategory:" + prodSizeId);
					
					System.out.println("Found sizeCat: " + sizeCat.getName());
					
					if (name != null && name.equals(sizeCat.getName())) {
						System.out.println("Matched ProductSizeCategory: " + sizeCat.getName());
						matchedSizeCat = sizeCat;
						break; // return immediately on first match
					}
				}
			}
		}
		if (matchedSizeCat == null) {
			System.out.println("No matching ProductSizeCategory found for name: " + name);
		}
		return matchedSizeCat;
//			if (prdSizeItr.hasNext()) {
//				FlexObject fobj = null;
//				fobj = (FlexObject) prdSizeItr.next();
//				String prodSizeId = fobj.getString("PRODUCTSIZECATEGORY.IDA2A2");
//
//				if (FormatHelper.hasContent(prodSizeId)) {
//					sizeCat = (ProductSizeCategory) LCSQuery
//							.findObjectById("com.lcs.wc.sizing.ProductSizeCategory:" + prodSizeId);
//					System.out.println("sizeCat "+sizeCat.getName());
//				if(name!=null&& name.equals(sizeCat.getName()))
//						{
//					System.out.println("sizeCat is match ::"+sizeCat);
//					return sizeCat;
//				}
//				}
//			}
...
//	public void measurement(LCSProduct product) throws WTException, WTPropertyVetoException {
//		System.out.println("--- measurement ---");
//		ProductSizeCategory sizeCategory = getProductSizeCategory(product);
//
//		LCSMeasurements lcm = new LCSMeasurements();
//		lcm.getProductMaster();
//		lcm.setProductSizeCategory(sizeCategory);
//
//		System.out.println("Set ProductSizeCategory: " + (sizeCategory != null ? sizeCategory.getName() : "null"));
//	}
