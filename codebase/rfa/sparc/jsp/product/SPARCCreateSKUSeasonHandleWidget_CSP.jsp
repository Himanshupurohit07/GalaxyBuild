...
<%--
    ...
    SPARCCreateSKUSeasonHandleWidget_CSP_CSP.jsp:  
 ...
 <%= csph.drawWidgetJsFunctionHandler(attKeyNamesToHandle)%>
	...
	function <%=csph.getJsFunctionName(COLORWAY_LIFECYCLE) %>(widgetObj){ 
		...
		if(throwEmptyOrNullCheck){
			...
			//vals.value= "scPlanned";//screleased
			var setOldSatusVal ;
			...
			if(vals.value === "scbuyready"){
				
				setOldSatusVal = "Planned";
			}
			...
			if(elem != null && elem != undefined && setOldSatusVal != null ){
				elem.textContent = setOldSatusVal;//span
			}
			vals.value= "scPlanned";
