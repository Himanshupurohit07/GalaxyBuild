...
<%--
    JSP Type: View
    $Source: /usr/local/cvsroot/lcsw/Windchill/web-root/jsp/sourcing/WhatIfCostSheet.jsp,v $

    $RCSfile: WhatIfCostSheet.jsp,v $: Updates a Sourcing Configuration.
    Current $Revision: 1.3 $  $Date: 2004/09/13 15:29:30 $   Build $Name:  $

    Revision Details are at the end of the file.
...
<%!
	...
	private static final Logger logger = LogManager.getLogger("rfa.sparc.jsp.sourcing.SPARCCopyCostSheet.jsp");
...
<%
   ...
   if(!sku.isPlaceholder()){
    ...
    String copyFromOid = request.getParameter("copyFromOid");
    LCSCostSheet from = (LCSCostSheet)LCSCostSheetQuery.findObjectById(copyFromOid);
    LCSSeason season = (LCSSeason) VersionHelper.latestIterationOf(from.getSeasonMaster()); 
    String seasonMstrId = "OR:com.lcs.wc.season.LCSSeasonMaster:" + Long.toString( (long) from.getSeasonMasterId());
...
/**********************Size Category*******************************************/
    ...
    Collection productSizeCats = new SizingQuery().findProductSizeCategoriesForProduct(productRevA).getResults();
...
/*****************************************************************/
    ...
    if(specMaster != null){
        FlexSpecToSeasonLink fstsl = FlexSpecQuery.findSpecToSeasonLink(specMaster, (LCSSeasonMaster)season.getMaster());
        if(fstsl != null){
            contextSpecId = FormatHelper.getObjectId(fstsl);
        }
    }
		// Changes to add the BOM Name from source Cost Sheet into Method Context - start
		String oldBOMValueFromCopyCostSheet = (String)csheet.getValue("scBOM");
		logger.debug("***oldBOMValueFromCopyCostSheet:"+oldBOMValueFromCopyCostSheet);
		if(oldBOMValueFromCopyCostSheet!=null && FormatHelper.hasContent(oldBOMValueFromCopyCostSheet)){
			MethodContext.getContext().putIfAbsent("oldBOMValueFromCopyCostSheet", oldBOMValueFromCopyCostSheet);
		}
...
<%-- /////////////////////////////////////// JAVSCRIPT ///////////////////////////////////--%>
<%-- /////////////////////////////////////////////////////////////////////////////////////--%>
<input name="csTypeId" type="hidden" value='<%= FormatHelper.getObjectId(type) %>'>
<%=FormGenerator.createHiddenInput("sourcingConfigId", FormatHelper.getVersionId(sconfig))%>
<%=FormGenerator.createHiddenInput("SKUId", FormatHelper.getVersionId(sku))%>
...
<script language="Javascript">
    ...
    function update(){
        ...
        <% if("COPY_COSTSHEET_TO_ACTIVE".equals(activity)){%>
            document.MAINFORM.activity.value = 'COPY_COSTSHEET_TO_ACTIVE';
        <% } else{%>
            document.MAINFORM.activity.value = 'COPY_COSTSHEET';
        <%}%>
            document.MAINFORM.action.value = 'SAVE';
            document.MAINFORM.seasonId.value = '<%= seasonMstrId %>';
            submitForm();
        }
    }
    function validate(){
...
<div class="themedPage">
   <div id="costSheetIdenfication" class="card">
 	...
 	<div class="card-header">
 		...
 		<%= costSheetIdentificationGrpTle %></label>
 	</div>
